...
import { Search, Globe, Github, FileText, CheckCircle2, AlertTriangle, Info, ShieldCheck, Cpu, Zap } from "lucide-react";
...
export function DocsGithubView({ insights }: { insights: Insight[] }) {
  const techInsights = insights.filter(i => i.category === "docs" || i.category === "github");
    ...
    <div className="mx-auto max-w-5xl py-12">
      <div className="mb-12 text-center">
        ...
        <h1 className="mb-4 text-4xl font-bold tracking-tight text-slate-900">
          Docs & GitHub Maturity
        </h1>
        <p className="mx-auto max-w-2xl text-lg text-slate-500">
          Analysis of your technical documentation quality and repository health signals.
        </p>
      </div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="lg:col-span-1 space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>Tech Maturity</CardTitle>
              <CardDescription>Overall technical presence score.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
                ...
                <div className="relative flex h-32 w-32 items-center justify-center rounded-full border-8 border-slate-100">
                  ...
                  <svg className="h-full w-full -rotate-90">
                    <circle
                      ...
                      strokeDashoffset={351.858 * (1 - 0.65)}
                      strokeLinecap="round"
                    />
                  </svg>
                </div>
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-500">Repo Trust Signals</span>
                  <Badge variant="success">High</Badge>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-slate-500">Developer Experience</span>
                  <Badge variant="warning">Medium</Badge>
                </div>
              </div>
            </CardContent>
        ...
        <div className="lg:col-span-2 space-y-6">
          <h2 className="text-xl font-bold text-slate-900">Technical Findings</h2>
          <div className="space-y-4">
            {techInsights.map((insight) => (
              <Card key={insight.id} className="overflow-hidden border-2 border-slate-100 hover:border-slate-900 transition-all">
                <CardContent className="p-6">
                  <div className="flex items-start gap-4">
                    <div className={cn("flex h-10 w-10 shrink-0 items-center justify-center rounded-xl", 
                      ...
                      {insight.severity === "critical" ? <AlertTriangle className="h-5 w-5" /> : <Cpu className="h-5 w-5" />}
                    </div>
